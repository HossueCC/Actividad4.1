...
Graph::Graph(int n) {
	nodos = n;
	adjList = new vector<int>[nodos];
	adjMatrix = new int [nodos*nodos];
...
void Graph::addEdgeAdjMatrix(int u, int v) {
    adjMatrix[u * nodos + v] = 1;
    adjMatrix[v * nodos + u] = 1;
    edgesMat++;
}
...
string Graph::DFS(int start, int target) {
    ...
    stack<int> my_stack;
    list<int> visited;
    vector<vector<int>> paths(nodos, vector<int>(1, -1));
    my_stack.push(start);
...
string Graph::BFS(int start, int target) {
    ...
    queue<int> queue;
    list<int> visited;
    vector<vector<int>> paths(nodos, vector<int>(0));
    queue.push(start);
